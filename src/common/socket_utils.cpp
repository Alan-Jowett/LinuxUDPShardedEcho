 ...
 * @brief Implementation of socket utilities and epoll helpers.
 *
 * This translation unit implements helpers for creating UDP sockets, binding sockets, making
 * non-blocking send/recv operations and working with epoll. It provides a thin platform-specific
 * layer used by the scalable echo server and client examples.
 *
...
void set_socket_cpu_affinity(const unique_fd& sock, uint16_t processor_id) {
    ...
    if (::setsockopt(sock.get(), SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) == -1) {
        throw socket_exception(
            std::format("setsockopt(SO_REUSEADDR) failed: {}", std::strerror(errno)));
    }
#ifdef SO_REUSEPORT
    if (::setsockopt(sock.get(), SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt)) == -1) {
        throw socket_exception(
            std::format("setsockopt(SO_REUSEPORT) failed: {}", std::strerror(errno)));
    }
#endif
}
...
void set_thread_affinity(uint32_t processor_id) {
    ...
    if (pthread_setaffinity_np(pthread_self(), sizeof(cpuset), &cpuset) != 0) {
        throw socket_exception(
            std::format("pthread_setaffinity_np failed: {}", std::strerror(errno)));
    }
}
